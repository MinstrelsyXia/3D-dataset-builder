...
def save_images(object_file: str) -> None:
    ...
    img_ids = [f"{view_num}" for view_num in range(24)]
    ...
    for index, image in enumerate(data["colors"]):
        render_path = os.path.join(args.output_dir, f"rgb_{img_ids[index]}.png")
        save_array_as_image(image, "colors", render_path)
    for index, depth in enumerate(data["depth"]):
        depth_path = os.path.join(args.output_dir, f"depth_{img_ids[index]}.exr")
